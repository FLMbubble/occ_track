...
def sort_files(detection_batch_dir,sort_out_dir='./detections/MY_PANOOCC'):
    ...
    for detection_f in detection_all:
        ...
        bev_vel=detection_data['vel'].squeeze()#W,H,2
...
def run(detection_file, timestamp_file,aux_file,ori_dir,output_file, min_confidence,max_cosine_distance,nn_budget, display,occ_size,occ_range):
    ...
    def frame_callback(vis, frame_idx):
        ...
        if not init_flag:
            ...
            prev_to_cur_ego=np.linalg.inv(cur_to_prev_ego.astype(np.float32))
...
def parse_args():
    ...
    parser.add_argument(
        "--detection_outdir", help="Path to detections results dir.", default='./detections/MY_PANOOCC',
        required=True)
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        required=True)
    parser.add_argument(
        "--aux_file", help="Path to auxiliary data.", default=None,
        required=True)
    parser.add_argument(
    ...
    parser.add_argument(
        "--occ_size", help="voxel resolution",
